...
def setup_args():
    ...
    parser.add_argument('--clean', action='store_true', help='Clean vector store before running')
...
def clean_vector_store(path):
    ...
    print(f"Cleaned vector store at: {path}")
        ...
        try:
            ...
            for page_num, page in enumerate(pdf_reader.pages):
                ...
                file_progress.progress(
                    ...
                    text=f"{progress_text} - Page {page_num + 1}/{total_pages}"
                )
...
def process_documents(pdf_docs, index_db_path):
    ...
    try:
        ...
        raw_text = get_pdf_text(pdf_docs, progress_bar)
...
def main():
    args = setup_args()
    load_dotenv()
    index_db_path = os.environ.get("FINANCIAL_REPORT_FAISS_INDEX_BAAI_MODEL_DIR")
    if not index_db_path:
        st.error("FINANCIAL_REPORT_FAISS_INDEX_BAAI_MODEL_DIR environment variable not set")
        return
    
    os.makedirs(index_db_path, exist_ok=True)
    
    if args.clean:
        clean_vector_store(index_db_path)
    ...
    <div style="background-color: #f0f2f6; padding: 10px; border-radius: 5px; margin-bottom: 20px;">
        <small>Using optimized BAAI/bge-small-en-v1.5 for fast processing and efficient retrieval</small>
    </div>
    """, unsafe_allow_html=True)
    
    user_question = st.text_input("Ask a question about company or stocks:")
